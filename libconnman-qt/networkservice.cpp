...
{
    QString val = m_priv->stringValue(Private::WPA3SAEPWE);

    return val.isEmpty() ? "default" : val;
}

void NetworkService::setWpa3SaePwe(const QString &wpa3SaePwe)
{
    if (wpa3SaePwe == "default")
        m_priv->setProperty(Private::WPA3SAEPWE, "");
    else
        m_priv->setProperty(Private::WPA3SAEPWE, wpa3SaePwe);
}
